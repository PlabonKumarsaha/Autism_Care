...
<?php
   ...
   $doctorID= $_SESSION['doctorID'];
    ...
    /*  $appID = $row[appointmentsId];
    $sqlName = "select userName from user where appointments == '$appID' ";
     ...
     if (!isset($_POST['upcomming']))
    {
        if (!isset($_POST['past']))
        {
         ...
         {
             $todayDate = date('d/M/Y', time() - 86400);
         }
     }
    else
    {
...
/*$str = "17-may-2018/anna/9.pm";
$first = explode("/", $str)[2]; */
 if (isset($_POST['confirm']))
 {
     $id = $_POST['id'];
     $query="UPDATE appointments SET appointmentsStatus='1' WHERE appointmentsID='$id'";
		$conn->query($query);
 }
if (isset($_POST['decline']))
 {
     $id = $_POST['id'];
     $query="UPDATE appointments SET appointmentsStatus='2' WHERE appointmentsID='$id'";
		$conn->query($query);
 }

$sql = "select * from appointments where doctorID ='$doctorID' and patientId > 0";
    ...
    <div class="container">
        ...
        <div class="right">
            <form method="post">
                <div class="shedule">
                    <form method="get">
                        <input type="submit" value="Today" id="today" class="btn btn-success" name="today">

                        <input type="submit" value="Upcomming" class="btn btn-success" name="upcomming">

                        <input type="submit" value="Past" class="btn btn-success" name="past">
                    </form>
                </div>
            </form>
